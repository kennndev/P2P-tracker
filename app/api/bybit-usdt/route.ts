...
export async function POST() {
    try {
        ...
        if (data.result?.items && data.result.items.length > 0) {
            ...
            console.log('✅ Bybit USDT P2P Data:', result);
        ...
        console.error('❌ Bybit USDT API Error:', error);
        return NextResponse.json(
            { error: 'Failed to fetch Bybit USDT data', details: error instanceof Error ? error.message : 'Unknown error' },
            { status: 500 }
        );
    }
}
